...
.to_top:
    sec
    lda A0                                                              ; A0 holds MSN, A1 holds LSN
    sbc #$03
    sta A0
    sec
    lda A1
...
.seek_rom:
    ...
    lda A0                                                              ; Get the last byte since loop doesn't catch the last byte
    pha
...
.store_nibble:
    pha                                                                 ; Push nibble back onto stack
    ...
    sta LCD_RAM+$12, x                                                  ; This is for storing in lower line -> X register holds the offset from ROM_MAP that we calculated earlier
    jmp .exit_store

.store_upper_line
    pla                                                                 ; Pull MSN from stack
    sta LCD_RAM+$01,x                                                   ; Store in upper line with X register offset calculated from before
...
.end:
    lda #":"
    sta LCD_RAM+$5
    sta LCD_RAM+$16                                                     ; Place colon 16 sections apart
    lda #"$"
    sta LCD_RAM
    sta LCD_RAM+$11
    rts
